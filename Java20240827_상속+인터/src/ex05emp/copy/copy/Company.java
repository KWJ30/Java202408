...
//		Sales sales = (Sales) emp[1];
		...
		Employee secretary = new Secretary("Duke", 1, "Secretary", 800);
		Employee sales = new Sales("Tuxi", 2, "sales", 1200);
		...
		if(isTax) {
			...
			for(Employee emps : emp) {
				...
				System.out.printf("%s\t %8s\t %8d\t \n",
						emps.getName(), emps.getDepartment(), emps.getSalary());
			}
			}
			...
			for(Employee emps : emp){
				...
				System.out.printf("%s\t %8s\t %8d\t %8.1f\t\n",
						emps.getName(), emps.getDepartment(),emps.getSalary(),emps.tax());			
			}
			}
